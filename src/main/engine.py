...
class Engine(threading.Thread):
    ...
    def __start_next_scenenario(self, map_name=None):
        ...
        if self.scenenario_index < len(self.scenenario_list):
            ...
            if not map_name:
                ...
                self.carla_adapter.set_map(self.current_scenenario.get_map().get_map_name())
            else:
    ...
    def check_assertion(self, trace_list):
        ...
        for trace in trace_list:           
            ...
            for item in detection_assertions:
                ...
                assertion_result = self.__set_detection_assertion(detection_assertion)
                if assertion_result:
                    self.assertion.append(assertion_result)
            # SafetyAssertions
            for item in safety_assertions:
                safety_assertion = item.get_assertions()
                assertion_result = self.__set_safety_assertion(safety_assertion)
                if assertion_result:
                    self.assertion.append(assertion_result)
        ...
        for item in self.trace:
            ...
            for perception_key in item['perception']:
                ...
                if len(perception_item) == 4:
                    ...
                    perception_array = np.zeros([len(perception_item), len(max(perception_item,key = lambda x: len(x)))])
                    ...
                    for ground_key in ground:
                        ...
                        if len(ground_item) == 4:
                            ...
                            ground_array = np.zeros([len(ground_item), len(max(ground_item,key = lambda x: len(x)))])
                            for i, j in enumerate(ground_item):
                                ground_array[i][0:len(j)] = j
                            
                            # print(ground_array, perception_array)
                            dis = np.linalg.norm(ground_array - perception_array)
                            # print(dis)
    ...
    def __set_detection_assertion(self, detections):
        ...
        for detection in detections:
            if isinstance(detection, AgentVisibleDetectionAssertion):
                ...
                trace_time = detection.get_agent_ground_distance().get_agent_ground_truth().get_trace_time().get_time()
                # 取出 dis(trace[1][ego], trace[1][truth][npc1])中trace[1][truth][npc1]的 npc1
                dis_agent_ground = detection.get_agent_ground_distance().get_agent_ground_truth().get_agent().get_name()
                # 取出 dis()<= 0.1 的 0.1
                ...
                diff_agent_state = detection.get_agent_error().get_agent_state().get_agent().get_name()
                # 取出 diff(trace[1][perception][npc1], trace[1][truth][npc1])中trace[1][truth][npc1]的 npc1
                diff_agent_ground = detection.get_agent_error().get_agent_ground_truth().get_agent().get_name()
                # 取出 diff()<= 0.1 的 0.1
                ...
                pass
        print(trace_time, dis_agent_ground, sensing_range, diff_agent_state, diff_agent_ground, error_threshold)
        # print(self.trace_modified)
        # 调用MTL
        trace_mtl = {
            'a': [],
        ...
        for item in self.trace_modified:
            ...
            if item['time'] >= trace_time and dis_agent_ground in item['truth'] and diff_agent_state in item['perception'] and diff_agent_ground in item['truth']:
                # dis
                trace_mtl['a'].append((item['time'], utils.dis(item['ego'][0], item['truth'][dis_agent_ground][0]) <= sensing_range))
                # diff
                trace_mtl['b'].append((item['time'], utils.diff(item['perception'][diff_agent_state], item['truth'][diff_agent_ground]) <= error_threshold))
        # print(trace_mtl)
    ...
    def __set_safety_assertion(self, safeties):
        ...
        for safety in safeties:
            if isinstance(safety, AgentVisibleDetectionAssertion):
                # 取出 dis(trace[1][ego], trace[1][truth][npc1])中trace[1][truth][npc1]的 1 -> 此处存疑：trace_time是否只取一次并全局使用
                trace_time = safety.get_agent_ground_distance().get_agent_ground_truth().get_trace_time().get_time()
                # 取出 dis(trace[1][ego], trace[1][truth][npc1])中trace[1][truth][npc1]的 npc1
                dis_agent_ground = safety.get_agent_ground_distance().get_agent_ground_truth().get_agent().get_name()
                # 取出 dis()<= 0.1 的 0.1
                ...
                # 取出 diff(trace[1][perception][npc1], trace[1][truth][npc1])中trace[1][truth][npc1]的 npc1
                diff_agent_ground = safety.get_agent_error().get_agent_ground_truth().get_agent().get_name()
                # 取出 diff()<= 0.1 的 0.1
        ...
        print(trace_time, dis_agent_ground, sensing_range, diff_agent_state, diff_agent_ground, error_threshold, safety_agent_state, safety_radius)
        ...
        for item in self.trace_modified:
            ...
            if item['time'] >= trace_time and dis_agent_ground in item['truth'] and diff_agent_state in item['perception'] and diff_agent_ground in item['truth'] and safety_agent_state in item['perception']:
                ...
                trace_mtl['b'].append((item['time'], utils.diff(item['perception'][diff_agent_state], item['truth'][diff_agent_ground]) <= error_threshold))
                # SafetyAssertion
                trace_mtl['c'].append((item['time'], utils.dis(item['ego'][0], item['perception'][safety_agent_state][0]) >= safety_radius))
    ...
    def __set_intersection_assertion(self, intersection):
        ...
        for item in self.trace_modified:
            ...
            if item['time'] >= trace_time:
                trace_mtl['a'].append((item['time'], item['truth']['traffic'] == item['perception']['traffic']))
                trace_mtl['b'].append((item['time'], item['perception']['traffic'] == (100, 200)))  # 判断红灯
                trace_mtl['c'].append((item['time'], np.linalg.norm(item['ego'][2])))
                trace_mtl['d'].append((item['time'], item['perception']['traffic'] == (100, 100)))  # 判断绿灯
    ...
    def __set_speed_constraint_assertion(self, speed):
        ...
        for item in self.trace_modified:
            ...
            if item['time'] >= trace_time:
                trace_mtl['a'].append((item['time'], item['truth']['traffic'] == item['perception']['traffic']))
                trace_mtl['b'].append((item['time'], item['perception']['traffic'] == (0, 2)))  # 检查速度范围
                trace_mtl['c'].append((item['time'], np.linalg.norm(item['ego'][2]) < item['perception']['traffic'][0]))  # 低速检查
                trace_mtl['d'].append((item['time'], np.linalg.norm(item['ego'][2]) > item['perception']['traffic'][1]))  # 超速检查
        ...
        phi = mtl.parse('G((a&b&(c|d))->(F[0,{}]~(c|d)))'.format(str(time_duration)))
        assertion_result_list = phi(trace_mtl, time=None, quantitative=False)
